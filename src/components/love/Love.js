...
import birthdaySong from './Happy-Birthday-To-You-4.mid';
import useSEO from '../../hooks/useSEO';

const PASSWORD = 'aadu';

const getMediaType = (path) => /\.(mp4|webm|mov)$/i.test(path) ? 'video' : 'image';
const importMedia = (context, group) => context.keys().sort().map((key, index) => ({
    id: `${group}-${index}`,
    src: context(key),
...
const togetherPhotos = importMedia(require.context('./photos/together', false, /\.(png|jpe?g|webp|gif)$/i), 'together');
const cutiePhotos = importMedia(require.context('./photos/cutie', false, /(\.(png|jpe?g|webp|gif)$|-web\.mp4$)/i), 'cutie');
const weirdPhotos = importMedia(require.context('./photos/weired', false, /\.(png|jpe?g|webp|gif)$/i), 'weird');
...
export default function Love() {
    ...
    const fallingPetals = useMemo(() => Array.from({ length: 34 }, (_, index) => ({
        id: index,
        left: `${(index * 19) % 100}%`,
    ...
    const playMusic = async () => {
        ...
        try {
            ...
            setMusicMessage('birthday music is playing');
        } catch (error) {
    ...
    const unlockSurprise = async (event) => {
        ...
        if (password.trim().toLowerCase() !== PASSWORD) {
            setPasswordError('Not this one. Hint: the daughter name I once told you.');
            return;
        }
        ...
        await playMusic();
        setTimeout(() => {
            document.getElementById('wish-garden')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }, 450);
    };
    ...
    return (
        <Box className={Style.lovePage} id={'love'}>
            <audio ref={audioRef} src={birthdaySong} loop onPause={() => setMusicPlaying(false)} onPlay={() => setMusicPlaying(true)} />
            ...
            <section className={Style.hero}>
                ...
                >
                    ...
                    <svg className={Style.roseSvg} viewBox="0 0 260 360" role="img" aria-label="A rose growing and blooming">
                        <defs>
                            ...
                            <filter id="softRoseShadow" x="-35%" y="-35%" width="170%" height="170%">
                                <feDropShadow dx="0" dy="16" stdDeviation="12" floodColor="#8b0d42" floodOpacity="0.35" />
                            </filter>
                        </defs>
                        <ellipse className={Style.roseShadow} cx="130" cy="333" rx="58" ry="12" />
                        <path className={Style.roseStemPath} d="M130 324 C126 275 137 241 128 198 C122 166 126 139 133 110" />
                        <path className={Style.roseLeafOne} d="M126 238 C79 211 59 228 38 268 C82 269 111 260 126 238Z" />
                        <path className={Style.roseLeafLineOne} d="M48 262 C79 251 102 245 126 238" />
                        <path className={Style.roseLeafTwo} d="M133 218 C177 183 206 198 222 235 C184 242 153 238 133 218Z" />
                        <path className={Style.roseLeafLineTwo} d="M213 232 C182 224 156 220 133 218" />
                        <g className={Style.roseBloom} filter="url(#softRoseShadow)">
                            <path className={Style.outerPetalOne} d="M130 116 C82 70 50 101 52 153 C54 205 94 223 128 192 C101 180 95 144 130 116Z" />
                            <path className={Style.outerPetalTwo} d="M130 116 C178 70 210 101 208 153 C206 205 166 223 132 192 C159 180 165 144 130 116Z" />
                            <path className={Style.outerPetalThree} d="M130 105 C93 45 124 25 163 54 C198 80 198 137 149 157 C153 136 145 118 130 105Z" />
                            <path className={Style.outerPetalFour} d="M130 105 C167 45 136 25 97 54 C62 80 62 137 111 157 C107 136 115 118 130 105Z" />
                            <path className={Style.middlePetalOne} d="M129 109 C91 97 80 137 100 171 C116 198 151 195 160 159 C135 160 121 140 129 109Z" />
                            <path className={Style.middlePetalTwo} d="M132 109 C169 96 182 137 160 171 C143 197 110 193 100 158 C125 160 140 139 132 109Z" />
                            <path className={Style.innerPetalOne} d="M130 94 C108 109 107 143 128 160 C148 142 150 112 130 94Z" />
                            <path className={Style.innerPetalTwo} d="M130 96 C116 82 96 88 89 110 C105 113 122 108 130 96Z" />
                            <path className={Style.innerPetalThree} d="M130 96 C144 82 164 88 171 110 C155 113 138 108 130 96Z" />
                            <path className={Style.roseCenter} d="M130 103 C113 114 116 138 130 148 C145 138 147 115 130 103Z" />
                            <path className={Style.roseHighlight} d="M102 104 C88 122 88 151 103 171" />
                            <path className={Style.roseHighlightTwo} d="M159 82 C177 100 181 130 170 153" />
                        </g>
                    </svg>
                    <div className={Style.roseCaption}>a little rose blooming for you</div>
                </motion.div>
                ...
                >
                    <Typography component="p" className={Style.eyebrow}>a birthday surprise</Typography>
                    <Typography component="h1">Happy Birthday, My Love</Typography>
                    <Typography component="p" className={Style.subtitle}>
                        I made this small garden of wishes for you. Open it slowly, one smile at a time.
                    </Typography>
                    <Button
                        variant="contained"
                ...
                {started && (
                    ...
                    >
                        <div className={Style.passwordPanel}>
                            ...
                            <Typography component="p">Only the birthday girl knows the key to this little garden.</Typography>
                            <Box component="form" onSubmit={unlockSurprise} className={Style.passwordForm}>
                                <TextField
                                    type="password"
                                    value={password}
                                ...
                                />
                                <Button type="submit" variant="contained" startIcon={<AutoAwesomeIcon />} className={Style.primaryButton}>
                                    Let it bloom
                                </Button>
                            </Box>
                        </div>
                    ...
                    >
                        <section id="wish-garden" className={Style.storySection}>
                            <div className={Style.sectionHeader}>
                                <FavoriteIcon />
                                <Typography component="h2">A garden of wishes for you</Typography>
                            </div>
                            ...
                            <div className={Style.polaroidWall}>
                                {togetherPhotos.map((photo, index) => (
                                    <motion.button
                                        ...
                                        whileInView={{ opacity: 1, y: 0, rotate: ((index % 5) - 2) * 2.8 }}
                                        viewport={{ once: true, margin: '-80px' }}
                                        transition={{ delay: index * 0.06, type: 'spring', stiffness: 110, damping: 15 }}
                        ...
                        <section className={Style.cutieSection}>
                            ...
                            <Typography component="p">Tap any card. Some smiles deserve a little flip.</Typography>
                                        ...
                                        <span className={Style.flipInner}>
                                            <span className={Style.flipFront}>
                                                {photo.type === 'video' ? (
                                                    ...
                                                    <img src={photo.src} alt="A cute memory" loading="lazy" />
                                                )}
                                            </span>
                                            <span className={Style.flipBack}>
                                                <FavoriteIcon />
                                                <strong>Too cute</strong>
                                                <small>and still my favorite</small>
                                            </span>
                                        </span>
                                    </motion.button>
                                ))}
                        ...
                        <section className={Style.weirdSection}>
                            ...
                            <Typography component="p">A tiny archive of weird, cute, and very important proof.</Typography>
                            <div className={Style.evidenceBoard}>
                                {weirdPhotos.map((photo, index) => (
                                    <motion.button
                                        ...
                                        whileHover={{ rotate: index % 2 === 0 ? 2 : -2, scale: 1.04 }}
                                        onClick={() => setSelectedMemory(photo)}
                        ...
                        <section className={Style.petalSection}>
                            ...
                            <Typography component="p">Every petal has something I wanted to say to you.</Typography>
                        ...
                        <section className={Style.heartSection}>
                            ...
                            <Typography component="p">A small heartbeat, drawn slowly for you.</Typography>
                            <svg className={Style.heartSvg} viewBox="0 0 320 290" role="img" aria-label="Animated heart forming">
                                <path
                        ...
                        <section className={Style.letterFolderSection}>
                            ...
                            <Typography component="p">Tap the folder to open the card I wrote for you.</Typography>
                            <button type="button" className={Style.folderButton} onClick={() => setShowFolderLetter(true)}>
                        ...
                        <section className={Style.trickySection}>
                            ...
                            <Typography component="p">One button is shy. One button is dramatic. Both are yours.</Typography>
                            <div className={Style.buttonLab}>
                                ...
                                <Button variant="contained" onClick={() => setShowLetter(true)} className={Style.dangerButton}>
                                    Open my heart
                                </Button>
                            </div>
                            ...
                            <AnimatePresence>
                                {showGift && (
                                    ...
                                    >
                                        Gift unlocked: endless support, warm hugs, and a boyfriend who is always cheering for you.
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            <AnimatePresence>
                                {showLetter && (
                                    ...
                                    >
                                        <button type="button" onClick={() => setShowLetter(false)}>x</button>
                                        <strong>My birthday letter</strong>
                                        <span>
                                            I am proud of the woman you are becoming. Your dedication, your softness,
                                            your dreams, and your smile make you so special to me. Happy Birthday, my love.
                                        </span>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        ...
                        <section className={Style.finalReveal}>
                            ...
                            >
                                <Typography component="p" className={Style.finalLabel}>14 August</Typography>
                                <Typography component="h2">Happy Birthday to You</Typography>
                                <Typography component="p">
                                    May your day be filled with flowers, blessings, laughter, and the quiet confidence
                                    that everything beautiful meant for you is finding its way to you.
                                </Typography>
                            </motion.div>
                                ...
                                {allMemories.slice(0, 42).map((memory, index) => (
                                    ...
                                    >
                                        {memory.type === 'video' ? (
                                            ...
                                            <img src={memory.src} alt="Birthday memory" loading="lazy" />
                                        )}
                                    </button>
                                ))}
                            </div>
                        ...
                        <AnimatePresence>
                            {showFolderLetter && (
                                ...
                                >
                                    ...
                                    >
                                        <button type="button" onClick={() => setShowFolderLetter(false)}>x</button>
                                        <span>My dearest love,</span>
                                        <p>
                                            On your birthday, I just want you to know how special you are to me.
                                            I love your strength, your innocence, your dreams, and the way you keep trying
                                            even when things feel heavy. I hope this year brings you peace, success,
                                            and every beautiful thing your heart quietly prays for.
                                        </p>
                                        <strong>Happy Birthday. I am always proud of you.</strong>
                                    </motion.article>
                                </motion.div>
                            )}
                        </AnimatePresence>
                        ...
                        <AnimatePresence>
                            {selectedMemory && (
                                ...
                                >
                                    ...
                                    >
                                        <button type="button" onClick={() => setSelectedMemory(null)}>x</button>
                                        {selectedMemory.type === 'video' ? (
                                            <video src={selectedMemory.src} controls autoPlay playsInline />
                                        ) : (
                                            <img src={selectedMemory.src} alt="Selected memory" />
                                        )}
                                        <span>one more reason this page is only yours</span>
                                    </motion.div>
                                </motion.div>
                            )}
                        </AnimatePresence>
            ...
            <button type="button" className={Style.musicButton} onClick={toggleMusic} aria-label="Toggle birthday music">
                <MusicNoteIcon />
